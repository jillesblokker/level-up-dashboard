...
-- STEP 2: Create quest_favorites table (run this second)
...
-- STEP 3: Create kingdom_grid table (run this third)
...
-- STEP 4: Create user_progress table (run this fourth)
...
-- STEP 5: Create achievements table (run this fifth)
...
-- STEP 6: Create tile_placements table (run this sixth)
...
-- STEP 7: Create monster_spawns table (run this seventh)
...
-- STEP 8: Create achievement_definitions table (run this eighth)
...
-- STEP 9: Add unique constraints (run this ninth)
...
ALTER TABLE user_progress ADD CONSTRAINT user_progress_user_unique UNIQUE (user_id);
ALTER TABLE achievements ADD CONSTRAINT achievements_user_achievement_unique UNIQUE (user_id, achievement_id);
ALTER TABLE tile_placements ADD CONSTRAINT tile_placements_user_position_unique UNIQUE (user_id, x, y);

-- STEP 10: Enable RLS (run this tenth)
...
-- STEP 11: Create RLS policies (run this eleventh)
...
CREATE POLICY "user_progress_delete_policy" ON user_progress FOR DELETE TO authenticated USING (user_id = auth.uid());

CREATE POLICY "achievements_select_policy" ON achievements FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY "achievements_insert_policy" ON achievements FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY "achievements_update_policy" ON achievements FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
CREATE POLICY "achievements_delete_policy" ON achievements FOR DELETE TO authenticated USING (user_id = auth.uid());

CREATE POLICY "tile_placements_select_policy" ON tile_placements FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY "tile_placements_insert_policy" ON tile_placements FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY "tile_placements_update_policy" ON tile_placements FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
CREATE POLICY "tile_placements_delete_policy" ON tile_placements FOR DELETE TO authenticated USING (user_id = auth.uid());

CREATE POLICY "monster_spawns_select_policy" ON monster_spawns FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY "monster_spawns_insert_policy" ON monster_spawns FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY "monster_spawns_update_policy" ON monster_spawns FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
CREATE POLICY "monster_spawns_delete_policy" ON monster_spawns FOR DELETE TO authenticated USING (user_id = auth.uid());

CREATE POLICY "achievement_definitions_read_policy" ON achievement_definitions FOR SELECT TO authenticated USING (true);

-- STEP 12: Create indexes (run this twelfth)
...
-- STEP 13: Grant permissions (run this thirteenth)
...
-- STEP 14: Insert achievement definitions (run this fourteenth)
