-- ==========================================
-- DEPLOY SMART QUEST COMPLETION SYSTEM (FIXED)
-- Run this in your Supabase SQL Editor
-- This version cleans up existing data first
-- ==========================================

-- 1. First, let's see what we're working with
DO $$ 
BEGIN
    RAISE NOTICE '🚀 Starting Smart Quest Completion System deployment...';
    RAISE NOTICE '📊 Current quest_completion table status:';
END $$;
...
-- 2. CLEAN UP EXISTING DATA FIRST (This is the key fix!)
DO $$ 
BEGIN
...
WHERE completed = true 
    AND (completed_at IS NULL OR xp_earned IS NULL OR xp_earned = 0 OR gold_earned IS NULL OR gold_earned = 0);

-- 3. Now add the CHECK constraint (should work after cleanup)
DO $$ 
BEGIN
...
-- 4. Create the smart quest completion function
...
-- 7. Create the clean quest completions view
...
-- 9. Verify the smart system is working
DO $$ 
DECLARE
