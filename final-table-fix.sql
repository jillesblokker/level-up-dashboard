-- Final Table Fix Script
-- This script aggressively handles existing constraints and forces table recreation
...
DECLARE
    r RECORD;
BEGIN
    -- Drop all constraints on quest_favorites
    FOR r IN (
        SELECT conname 
        FROM pg_constraint 
        WHERE conrelid = 'quest_favorites'::regclass
    ) LOOP
        EXECUTE 'ALTER TABLE quest_favorites DROP CONSTRAINT IF EXISTS ' || quote_ident(r.conname) || ' CASCADE';
    END LOOP;
    
    -- Drop all constraints on kingdom_grid
    FOR r IN (
        SELECT conname 
        FROM pg_constraint 
        WHERE conrelid = 'kingdom_grid'::regclass
    ) LOOP
        EXECUTE 'ALTER TABLE kingdom_grid DROP CONSTRAINT IF EXISTS ' || quote_ident(r.conname) || ' CASCADE';
    END LOOP;
    
    -- Drop all constraints on user_progress
    FOR r IN (
        SELECT conname 
        FROM pg_constraint 
        WHERE conrelid = 'user_progress'::regclass
    ) LOOP
        EXECUTE 'ALTER TABLE user_progress DROP CONSTRAINT IF EXISTS ' || quote_ident(r.conname) || ' CASCADE';
    END LOOP;
    
    -- Drop all constraints on achievements
    FOR r IN (
        SELECT conname 
        FROM pg_constraint 
        WHERE conrelid = 'achievements'::regclass
    ) LOOP
        EXECUTE 'ALTER TABLE achievements DROP CONSTRAINT IF EXISTS ' || quote_ident(r.conname) || ' CASCADE';
    END LOOP;
    
    -- Drop all constraints on tile_placements
    FOR r IN (
        SELECT conname 
        FROM pg_constraint 
        WHERE conrelid = 'tile_placements'::regclass
    ) LOOP
        EXECUTE 'ALTER TABLE tile_placements DROP CONSTRAINT IF EXISTS ' || quote_ident(r.conname) || ' CASCADE';
    END LOOP;
    
    -- Drop all constraints on monster_spawns
    FOR r IN (
        SELECT conname 
        FROM pg_constraint 
        WHERE conrelid = 'monster_spawns'::regclass
    ) LOOP
        EXECUTE 'ALTER TABLE monster_spawns DROP CONSTRAINT IF EXISTS ' || quote_ident(r.conname) || ' CASCADE';
    END LOOP;
...
-- Force drop tables with CASCADE
...
-- Add unique constraints with different names to avoid conflicts
