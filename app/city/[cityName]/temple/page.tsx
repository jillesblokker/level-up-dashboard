...
import { CityItemManager, MagicItem } from "@/lib/city-item-manager"
...
export default function TemplePage() {
  ...
  // Temple items
  const [templeItems] = useState<MagicItem[]>(CityItemManager.getTempleItems())

  // Purchase an item
  const purchaseItem = (item: MagicItem) => {
    ...
    <div className="flex min-h-screen flex-col bg-black text-white">
      <main className="flex-1 p-4 md:p-6 space-y-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold tracking-tight font-serif">The Sacred Temple</h1>
        ...
        <div className="relative w-full h-[250px] rounded-lg overflow-hidden border-2 border-amber-800/20 mb-8">
          <div className="absolute inset-0 bg-gradient-to-b from-amber-900/30 to-black/70">
            <div className="absolute inset-0 flex flex-col items-center justify-center text-white">
              <h2 className="text-3xl font-bold mb-2 font-serif">Temple of Light</h2>
              <p className="text-lg">Sacred artifacts and divine blessings</p>
            </div>
          </div>
        </div>
        ...
        <Card className="bg-gradient-to-b from-black to-gray-900 border-amber-800/20">
          <CardHeader>
            ...
            <CardDescription>Blessed artifacts and magical scrolls</CardDescription>
          </CardHeader>
          <CardContent>
