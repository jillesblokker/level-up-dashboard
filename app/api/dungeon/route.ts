...
export async function POST(req: NextRequest) {
    try {
        ...
        if (action === 'start') {
            ...
            const { data: kingdomGrid } = await supabaseServer
                .from('kingdom_grid')
                .select('grid_data')
                .eq('user_id', userId)
                .single();
        ...
        if (action === 'play') {
            ...
            if (encounter.type === 'monster') {
                ...
                if (choice === 'use_item' && itemId) {
                    // --- Use Potion ---
                    const { data: itemToUse } = await supabaseServer
                        .from('inventory_items')
                        .select('*')
                        .eq('user_id', userId)
                        .eq('item_id', itemId)
            ...
            const { data: updatedRun, error } = await supabaseServer
                .from('dungeon_runs')
                .update({
                    current_hp: newHp,
            ...
            if (newStatus !== 'in_progress') {
                ...
                await processEndRun(supabaseServer, userId, newLoot, endBonuses);
                apiLogger.info(`Dungeon run ${runId} finished with status: ${newStatus}`);
            }

            return NextResponse.json({ ...updatedRun, message: resultMessage, actionResult: { damageTaken, lootFound } });
        } // End of action 'play'
