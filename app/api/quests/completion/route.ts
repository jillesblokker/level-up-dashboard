...
export async function POST(request: Request) {
  try {
    ...
    logger.debug('[API/quests/completion] Received body:', data);
    const { questId } = data;
    if (!questId) {
      logger.error('[API/quests/completion] Missing questId');
    ...
    if (smartError) {
      logger.error('[API/quests/completion] Smart completion error:', smartError);
      return NextResponse.json({ error: smartError.message }, { status: 500 });
    }

    logger.debug('[API/quests/completion] Smart completion result:', smartResult);
    ...
    if (smartResult && smartResult.record) {
      try {
        ...
        logger.debug('[API/quests/completion] Rewards granted successfully');
      } catch (rewardError) {
        logger.error('[API/quests/completion] Error granting rewards:', rewardError);
    ...
    logger.error('[API/quests/completion] Internal server error:', error);
...
export async function GET(request: Request) {
  try {
    logger.debug('[Quests Completion API] GET request received');
    ...
    try {
      ...
      logger.error('[Quests Completion API] Auth error:', authError);
      return NextResponse.json({ error: 'Authentication error' }, { status: 401 });
    }

    if (!userId) {
      logger.debug('[Quests Completion API] No userId found');
      return NextResponse.json({ error: 'Unauthorized (Clerk JWT invalid or missing)' }, { status: 401 });
    }

    logger.debug('[Quests Completion API] User authenticated:', userId);

    if (!supabase) {
      logger.error('[Quests Completion API] Supabase client not initialized');
      return NextResponse.json({ error: 'Supabase client not initialized.' }, { status: 500 });
    }
    ...
    if (error) {
      logger.error('[Quests Completion API] Error fetching quest completions:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    logger.debug('[Quests Completion API] Found', questCompletions?.length || 0, 'completions');
    return NextResponse.json(questCompletions || []);
  } catch (error) {
    logger.error('[Quests Completion API] Internal server error:', error);
...
export async function PUT(request: Request) {
  try {
    ...
    logger.debug('[QUESTS/COMPLETION][PUT] Request body:', body);

    const { questId, completed } = body;
    if (!questId || typeof completed !== 'boolean') {
      logger.error('[QUESTS/COMPLETION][PUT] Invalid request data:', { questId, completed });
      return NextResponse.json({ error: 'Missing questId or completed' }, { status: 400 });
    }

    logger.debug('[QUESTS/COMPLETION][PUT] Processing quest completion:', { userId, questId, completed });
    ...
    if (questError || !quest) {
      logger.error('[QUESTS/COMPLETION][PUT] Quest not found:', { questError, questId });
      return NextResponse.json({ error: 'Quest not found' }, { status: 404 });
    }

    logger.debug('[QUESTS/COMPLETION][PUT] Quest found:', { questId, xpReward: quest.xp_reward, goldReward: quest.gold_reward });
    logger.debug('[QUESTS/COMPLETION][PUT] Quest ID type:', typeof questId, 'Length:', questId?.length, 'Format:', questId);

    // 🚀 DAILY HABIT TRACKING SYSTEM - Create new completion record for today
    logger.debug('[QUESTS/COMPLETION][PUT] Using daily habit tracking system...');
    ...
    if (completed) {
      ...
      if (completionError) {
        ...
        if (completionError.code === '23505') { // Postgres duplicate key error code
          logger.debug('[QUESTS/COMPLETION][PUT] Duplicate completion detected, fetching existing record...');
          ...
          if (existingError || !existingData) {
            logger.error('[QUESTS/COMPLETION][PUT] Failed to fetch existing duplicate:', existingError);
            return NextResponse.json({ error: completionError.message }, { status: 409 });
          }
          questCompletion = existingData;
        } else {
          logger.error('[QUESTS/COMPLETION][PUT] Error creating completion record:', completionError);
          return NextResponse.json({ error: completionError.message }, { status: 500 });
        }
      } else {
        questCompletion = completionData;
      }
      logger.debug('[QUESTS/COMPLETION][PUT] New completion record created:', questCompletion);
    } else {
      ...
      if (fetchError || !todayCompletion) {
        logger.error('[QUESTS/COMPLETION][PUT] No completion record found for today:', fetchError);
      ...
      if (updateError) {
        logger.error('[QUESTS/COMPLETION][PUT] Error updating completion record:', updateError);
        return NextResponse.json({ error: updateError.message }, { status: 500 });
      }

      questCompletion = completionData;
      logger.debug('[QUESTS/COMPLETION][PUT] Completion record updated:', questCompletion);
    }
    ...
    if (completed) {
      try {
        ...
        logger.error('[QUESTS/COMPLETION][PUT] Error granting rewards:', rewardError);
    ...
    logger.error('[QUESTS/COMPLETION][PUT] Internal server error:', error);
