...
export async function GET(request: Request) {
  try {
    ...
    if (searchParams.get('test') === '1') {
      logger.debug('[ACHIEVEMENTS][TEST] Testing Supabase connection...');
      try {
        ...
        if (error) {
          logger.error('[ACHIEVEMENTS][TEST] Supabase test error:', error);
          return NextResponse.json({
            test: 'failed',
            error: error.message,
            code: error.code
        ...
        logger.error('[ACHIEVEMENTS][TEST] Test exception:', testError);
        return NextResponse.json({
          test: 'exception',
    ...
    if (searchParams.get('schema') === '1') {
      logger.debug('[ACHIEVEMENTS][SCHEMA] Testing table schema...');
      try {
        ...
        if (error) {
          logger.error('[ACHIEVEMENTS][SCHEMA] Schema test error:', error);
          return NextResponse.json({
            schema: 'error',
            error: error.message,
            code: error.code,
        ...
        logger.error('[ACHIEVEMENTS][SCHEMA] Schema test exception:', schemaError);
        return NextResponse.json({
          schema: 'exception',
    ...
    if (searchParams.get('debug') === '1') {
      logger.debug('[ACHIEVEMENTS][DEBUG] Testing Clerk auth...');
      try {
        const { userId } = await auth();
        logger.debug('[ACHIEVEMENTS][DEBUG] Clerk auth result:', { userId });
        ...
        logger.error('[ACHIEVEMENTS][DEBUG] Clerk auth error:', authError);
        return NextResponse.json({
          debug: 'auth_error',
    ...
    const { userId } = await auth();
    logger.debug('[ACHIEVEMENTS][GET] User ID from Clerk:', userId);

    if (!userId) {
      logger.debug('[ACHIEVEMENTS][GET] No userId found in auth');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    // Get userId from query params (for backward compatibility)
    const queryUserId = searchParams.get('userId');
    logger.debug('[ACHIEVEMENTS][GET] Query userId:', queryUserId);

    // Use query userId if provided, otherwise use auth userId
    const targetUserId = queryUserId || userId;

    logger.debug('[ACHIEVEMENTS][GET] Fetching achievements for user:', targetUserId);
    ...
    if (sampleError) {
      logger.error('[ACHIEVEMENTS][GET] Sample query error:', sampleError);
      return NextResponse.json({
    ...
    logger.debug('[ACHIEVEMENTS][GET] Sample data structure:', sampleData);
    ...
    if (error) {
      logger.error('[ACHIEVEMENTS][GET] Supabase query error:', error);
      return NextResponse.json({
        error: error.message,
        code: error.code,
        details: error.details,
    ...
    logger.debug('[ACHIEVEMENTS][GET] Raw data from Supabase:', data);
    ...
    logger.debug('[ACHIEVEMENTS][GET] Processed achievements:', achievements.length, 'items');

    const response = NextResponse.json(achievements);
    // Cache for 5 minutes with stale-while-revalidate — achievements don't change mid-session
    response.headers.set('Cache-Control', 'private, max-age=300, stale-while-revalidate=60');
    return response;
  } catch (error) {
    logger.error('[ACHIEVEMENTS][GET] Internal server error:', error instanceof Error ? error.stack : error);
    return NextResponse.json({
