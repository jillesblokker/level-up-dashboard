...
export async function POST(request: NextRequest) {
  try {
    logger.debug('[Restore September 16 Data] Starting restoration process');
    
    const { userId } = await auth();
    if (!userId) {
      logger.error('[Restore September 16 Data] Unauthorized');
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
    }

    logger.debug('[Restore September 16 Data] Authenticated user:', userId);
    ...
    if (favoriteError) {
      logger.error('[Restore September 16 Data] Error fetching favorited quests:', favoriteError);
    ...
    logger.debug('[Restore September 16 Data] Found favorited quest IDs:', favoritedQuestIds);
    ...
    logger.debug('[Restore September 16 Data] Fetched quest details:', favoritedQuests?.length || 0, 'quests');
    if (favoritedQuests && favoritedQuests.length > 0) {
      logger.debug('[Restore September 16 Data] Sample quest data:', favoritedQuests[0]);
    }

    if (questError) {
      logger.error('[Restore September 16 Data] Error fetching favorited quests:', questError);
      return NextResponse.json({ error: 'Failed to fetch favorited quests' }, { status: 500 });
    }

    logger.debug('[Restore September 16 Data] Found favorited quests:', favoritedQuests?.length || 0);
    ...
    if (existingError) {
      logger.error('[Restore September 16 Data] Error checking existing completions:', existingError);
    ...
    logger.debug('[Restore September 16 Data] Existing completions (any date):', existingQuestIds.length);
    ...
    logger.debug('[Restore September 16 Data] Quests to insert:', questsToInsert.length);
    logger.debug('[Restore September 16 Data] Quests to update:', questsToUpdate.length);
    ...
    for (const quest of questsToInsert) {
      try {
        ...
        if (insertError) {
          logger.error(`[Restore September 16 Data] Error inserting quest ${quest.name}:`, insertError);
          errors.push({ quest: quest.name, error: insertError.message });
          errorCount++;
        } else {
          logger.debug(`[Restore September 16 Data] Successfully inserted quest: ${quest.name}`);
          successCount++;
        }
      } catch (error) {
        logger.error(`[Restore September 16 Data] Exception for quest ${quest.name}:`, error);
    ...
    for (const quest of questsToUpdate) {
      try {
        ...
        if (updateError) {
          logger.error(`[Restore September 16 Data] Error updating quest ${quest.name}:`, updateError);
          errors.push({ quest: quest.name, error: updateError.message });
          errorCount++;
        } else {
          logger.debug(`[Restore September 16 Data] Successfully updated quest: ${quest.name}`);
          successCount++;
        }
      } catch (error) {
        logger.error(`[Restore September 16 Data] Exception for quest ${quest.name}:`, error);
    ...
    logger.debug(`[Restore September 16 Data] Restoration complete: ${successCount} successful, ${errorCount} errors`);
    ...
    logger.error('[Restore September 16 Data] Internal server error:', error);
