...
export async function POST(request: NextRequest) {
  try {
    ...
    if (amount === undefined || amount === null || totalAfter === undefined || totalAfter === null || !transactionType || !source) {
    ...
    const queryPromise = authenticatedSupabaseQuery(request, async (supabase, userId) => {
      ...
      if (error) {
        logger.error('[Experience Transactions API] Insert error:', error);
        throw error;
      }
    ...
    logger.error('[Experience Transactions API] Error:', error);
...
export async function GET(request: NextRequest) {
  try {
    ...
    const queryPromise = authenticatedSupabaseQuery(request, async (supabase, userId) => {
      ...
      if (error) {
        logger.error('[Experience Transactions API] Select error:', error);
        throw error;
      }
