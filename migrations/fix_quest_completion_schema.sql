-- FIX: Ensure quest_completion table supports UUIDs (alphanumeric) for quest_id
-- Standard quests use numbers, but custom quests use UUIDs.
-- We must convert the column to TEXT to support both.

-- 1. Alter table column type (this will preserve existing data as strings)
ALTER TABLE quest_completion ALTER COLUMN quest_id TYPE text;

-- 2. Drop and Recreate the RPC function to accept TEXT and handle both types
DROP FUNCTION IF EXISTS smart_quest_completion;

CREATE OR REPLACE FUNCTION smart_quest_completion(
    p_user_id text,
    p_quest_id text, -- Changed to TEXT to support both UUIDs and numeric IDs
    p_completed boolean,
