...
## Full Audited 53-Page Evaluation Matrix
...
| **13** | **Riddles & Labyrinth** | `/riddles` | Interactive minigame puzzles (Plank Labyrinth, Sphinx Riddles, Tarot Reading). | **9.0 / 10** | Interactive Plank Labyrinth puzzle solver & daily Sphinx riddle rewards. | **8.5 / 10** |
| **14** | **Card & Loot Reveal** | `/reveal` | High-dopamine card pack opening and Chrono Chest loot reveal animation. | **9.0 / 10** | Astral Fortune (+15% card odds) unowned card priority drops. | **8.0 / 10** |
...
| **19** | **World Map Spire** | `/worldmap` | High-level interactive world map displaying distant ports and realms. | **8.5 / 10** | Port trade route navigation & voyage ETA countdowns. | **7.5 / 10** |
| **20** | **Achievements** | `/achievements` | Lifetime cumulative milestones and achievement trophy room. | **9.0 / 10** | Cumulative milestone rewards & House Cup virtue stat bonuses. | **8.0 / 10** |
| **21** | **Notification Center** | `/notifications` | Action Feed for Dares, Titan Victories, Paragon level gains, and streak alerts. | **9.0 / 10** | 24h auto-archiving & one-tap action cards (Accept Dare, Claim Victory Chest). | **8.5 / 10** |
...
| **24** | **City Local Market** | `/city/[cityName]/shop` | City-specific merchant shop for regional goods and blueprints. | **7.5 / 10** | Regional blueprint shop using Ether coin currency. | **7.5 / 10** |
| **25** | **Town Overview** | `/town/[slug]` | Settlement waypoint view for regional towns across the realm. | **7.0 / 10** | Local daily bounty contracts & town reputation rewards. | **7.0 / 10** |
| **26** | **Town Location Detail** | `/town/[slug]/[locationId]` | Specific town landmark location drawer. | **6.5 / 10** | Town waypoint location drawers & citizen summaries. | **6.5 / 10** |
| **27** | **Location Detail** | `/locations/[id]` | Universal location view for dynamic world landmarks. | **6.5 / 10** | Map waypoint quick travel portal. | **6.5 / 10** |
| **28** | **Category Filter View** | `/category/[slug]` | Filtered view of quests and items by habit category (Might, Knowledge, etc.). | **8.0 / 10** | Category virtue stats & habit combat stat bonus preview (+5% ATK per Might habit). | **8.0 / 10** |
...
| **37** | **Account Stored Data** | `/account/stored-data` | Data export and privacy management. | **8.5 / 10** | One-click GDPR data JSON download & account export. | **7.0 / 10** |
| **38** | **Settings** | `/settings` | Global app preferences, sound toggles, and theme settings. | **9.0 / 10** | Audio volume sliders (BGM, SFX, Haptics) & notification preferences. | **7.5 / 10** |
| **39** | **Event Preferences** | `/settings/events` | Push notification and in-game event alert settings. | **8.0 / 10** | Push notification toggles for Titan Raids, Dares & Streaks. | **7.0 / 10** |
| **40** | **Hint Preferences** | `/settings/hints` | Tutorial prompts and helper hint toggle settings. | **8.0 / 10** | Reset tutorial hints button & tooltip density control. | **6.0 / 10** |
| **41** | **Habit Requirements** | `/settings/requirements` | Custom target settings for daily habit goals. | **8.0 / 10** | Custom daily target sweet spot configuration (default 5 habits). | **7.5 / 10** |
| **42** | **Hero Setup** | `/setup` | Initial onboarding flow for avatar and initial habit selection. | **9.0 / 10** | Streamlined 3-step onboarding picker with 50 Gold starter gift. | **8.5 / 10** |
...
| **45** | **Admin Stored Data** | `/admin/stored-data` | Administrative database inspector and table browser. | **8.0 / 10** | Table row count summary & raw JSON payload inspector. | **6.0 / 10** |
| **46** | **Restore Progress** | `/restore-progress` | Self-service player quest progress restoration tool. | **8.5 / 10** | Auto-detect missing habit completions & 1-tap repair tool. | **8.0 / 10** |
| **47** | **Restore Stats** | `/restore-stats` | Self-service player character stats restoration tool. | **8.5 / 10** | Level & gold integrity check with snapshot rollback. | **7.5 / 10** |
| **48** | **Merge Accounts Utility** | `/merge-users` | Account consolidation tool to merge duplicate profile IDs. | **8.0 / 10** | Profile merger preview modal showing combined gold, level, and quests. | **7.0 / 10** |
| **49** | **Migration Status Monitor** | `/migration-status` | Database schema migration status dashboard. | **8.0 / 10** | Supabase schema migration health check status badges. | **6.0 / 10** |
| **50** | **Design System Catalog** | `/design-system` | Developer showcase of UI tokens, buttons, cards, and modals. | **9.0 / 10** | UI component showcase for modals, toasts, and buttons. | **7.0 / 10** |
| **51** | **Images Asset Gallery** | `/images` | Asset browser for kingdom tiles and artwork images. | **8.0 / 10** | Kingdom tile image asset gallery. | **6.0 / 10** |
| **52** | **System Requirements** | `/requirements` | Browser compatibility and system requirements page. | **8.0 / 10** | System browser compatibility checklist. | **5.5 / 10** |
| **53** | **Test Defaults Sandbox** | `/test-defaults` | Developer testing ground for default state configurations. | **8.0 / 10** | One-click test data seeders for developer testing. | **7.5 / 10** |
