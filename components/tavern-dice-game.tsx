  ...
  playDiceRoll() {
    ...
    for (let i = 0; i < 6; i++) {
      setTimeout(() => {
        ...
        osc.type = 'square';
        osc.frequency.setValueAtTime(160 + Math.random() * 320, ctx.currentTime);
...
function computeAIDecision(
  ...
  if (difficulty === 'easy') challengeThreshold = 0.08;
  if (difficulty === 'hard' || difficulty === 'master') challengeThreshold = 0.32;
  ...
  if (probTrue < challengeThreshold) {
    if (difficulty === 'easy' && Math.random() < 0.25) {
      // Continue to raise on easy occasionally
    } else {
      return { action: 'challenge' };
    }
  }
  ...
  const scoredRaises = legalRaises.map(bid => {
    ...
    let bluffBonus = 0;
    if ((difficulty === 'hard' || difficulty === 'master') && held === 0 && Math.random() < 0.25) {
      bluffBonus = 0.15;
    }
...
export function TavernDiceGame() {
  ...
  // Unified Player Raise Selectors
  ...
  const startNewGame = async (tier: WagerTier) => {
    if (goldBalance < tier.wager) {
      toast({
        title: "Insufficient Gold!",
        description: `You need at least ${formatGold(tier.wager)} Gold to enter this table.`,
        variant: "destructive"
      });
      return;
    }
    ...
    if (playerLevel < tier.requiredLevel) {
      toast({
        title: "Table Locked!",
        description: `Unlocks at Player Level ${tier.requiredLevel}.`,
        variant: "destructive"
      });
      return;
    }
    ...
    startNewRound('player', 5, 5, tier.id);
  };
  ...
  const startNewRound = (starter: 'player' | 'computer', pCount: number, cCount: number, currentDiff: AIDifficulty) => {
    ...
    setTimeout(() => {
      ...
      if (starter === 'player') {
        ...
        triggerAITurn(cRolls, pCount, null, currentDiff, []);
      }
    }, 1300);
  };
  ...
  const handlePlayerRaise = () => {
    ...
    if (currentBid) {
      const isQtyHigher = selectedRaiseQty > currentBid.quantity;
      const isFaceHigherSameQty = selectedRaiseQty === currentBid.quantity && selectedRaiseFace > currentBid.face;
      const isValidRaise = isQtyHigher || isFaceHigherSameQty;

      if (!isValidRaise) {
        toast({
          title: "Invalid Raise!",
          description: `Your bid must be higher in quantity (${currentBid.quantity}+) or face value (${currentBid.face}+).`,
          variant: "destructive"
        });
        return;
      }
    ...
    const newBid: Bid = { quantity: selectedRaiseQty, face: selectedRaiseFace, bidder: 'player' };
    setCurrentBid(newBid);
    setBidHistory(prev => [newBid, ...prev]);
  ...
  const handleNextRound = () => {
    ...
    startNewRound(nextStarter, playerDiceCount, computerDiceCount, difficulty);
  };
        ...
        {phase !== 'difficultySelect' && (
          <div className="space-y-6">
            ...
            >
              ...
              <div className="relative z-10 bg-black/70 border border-amber-900/40 p-4 rounded-xl text-center space-y-3 shadow-inner backdrop-blur-sm">
                ...
                {phase === 'rolling' && (
                  <div className="text-xs text-amber-300 font-serif italic animate-pulse flex items-center justify-center gap-2">
                    <Dices className="w-4 h-4 animate-spin" /> Shaking dice cups onto the wooden table...
                  </div>
                )}
              </div>
            ...
            {/* 4. PLAYER CONTROLS & UNIFIED BIDDING INTERFACE */}
            {phase === 'playerTurn' && (
              <div className="bg-zinc-950 border border-amber-900/40 rounded-2xl p-5 space-y-4 shadow-xl animate-in zoom-in-95 duration-200">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="bg-zinc-900/90 border border-zinc-800 rounded-xl p-3 space-y-2">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-zinc-400 font-serif font-bold">1. Select Quantity</span>
                        <span className="font-mono font-bold text-amber-300">{selectedRaiseQty} Dice</span>
                      </div>
                        ...
                        {Array.from({ length: Math.min(10, totalDiceInPlay) }).map((_, i) => {
                          ...
                          return (
                            <Button
                              ...
                              className={`h-8 text-xs font-mono p-0 ${selectedRaiseQty === q ? 'bg-amber-600 hover:bg-amber-500 text-black font-bold' : ''}`}
                            >
                              {q}
                            </Button>
                          );
                    ...
                    <div className="bg-zinc-900/90 border border-zinc-800 rounded-xl p-3 space-y-2">
                      <div className="flex justify-between items-center text-xs">
                        <span className="text-zinc-400 font-serif font-bold">2. Select Face Value</span>
                        <span className="font-mono font-bold text-amber-300">Face {selectedRaiseFace}</span>
                      </div>
                      <div className="grid grid-cols-6 gap-1">
                        {[1, 2, 3, 4, 5, 6].map(f => (
                          <Button
                            ...
                            className={`h-8 text-xs font-mono p-0 ${selectedRaiseFace === f ? 'bg-amber-600 hover:bg-amber-500 text-black font-bold' : ''}`}
                          >
                            {f}
                          </Button>
                        ))}
                  ...
                  {/* UNIFIED SINGLE RAISE BID BUTTON & CHALLENGE LIAR BUTTON */}
            ...
            {phase === 'gameOver' && (
              <div className="bg-zinc-950 border-2 border-amber-500/60 rounded-2xl p-8 text-center space-y-6 shadow-2xl animate-in zoom-in-95 duration-500">
                {playerDiceCount > 0 ? (
                  <div className="space-y-3">
                    ...
                    <p className="text-sm text-zinc-300 max-w-md mx-auto">
                      You have outwitted {selectedTier.opponentName} and claimed your tavern prize!
                    </p>
                    <Badge className="bg-emerald-950 text-emerald-300 border border-emerald-500/40 text-sm px-4 py-1 font-mono">
                      +{(Math.floor(selectedTier.wager * selectedTier.multiplier))} Gold Payout 🪙
                    </Badge>
                  </div>
                ) : (
                  <div className="space-y-3">
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-4">
                  <Button
                    onClick={() => startNewGame(selectedTier)}
                    size="lg"
                    className="w-full sm:w-auto bg-amber-600 hover:bg-amber-500 text-black font-serif font-bold text-sm px-6"
                  >
                    <RotateCcw className="w-4 h-4 mr-2" /> Play Again ({formatGold(selectedTier.wager)} Gold)
                  </Button>
