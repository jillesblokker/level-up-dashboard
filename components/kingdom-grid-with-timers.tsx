...
export function KingdomGridWithTimers({
  ...
  // Function to fetch property timers from API or fallback to user-scoped storage
  const fetchPropertyTimers = useCallback(async () => {
    let loadedTimers: TileTimer[] = [];
    let success = false;
    try {
    ...
    if (!success) {
      ...
      try {
        const savedTimers = getUserScopedItem('kingdom-tile-timers')
        if (savedTimers) {
    ...
    // Check the grid. For any placed tile with timerMinutes > 0,
    // if it does not have a timer in loadedTimers, initialize it.
    const finalTimers = [...loadedTimers];
    const initialNewTimers: TileTimer[] = [];
    let didAddDefaults = false;
  ...
  // Load timers on mount or when grid changes
  useEffect(() => {
    fetchPropertyTimers();
  }, [fetchPropertyTimers]);

  // Save timers to user-scoped storage whenever they change
  useEffect(() => {
    if (tileTimers.length > 0) {
      setUserScopedItem('kingdom-tile-timers', JSON.stringify(tileTimers))
    }
  }, [tileTimers])

  // Update timer readiness every 10 seconds & re-fetch from API when tab becomes visible
  useEffect(() => {
  ...
  const handleTileClick = (x: number, y: number, tile: Tile) => {
    ...
    if (tile.type && (tile.type === 'blacksmith' || tile.type === 'sawmill' ||
      ...
      // Handle Mystic Obelisk special effect
  ...
  useEffect(() => {
    const handleGlobalCollect = () => {
      handleCollectAllReady();
    };
    window.addEventListener('collect-all-kingdom-taxes', handleGlobalCollect);
    return () => window.removeEventListener('collect-all-kingdom-taxes', handleGlobalCollect);
  }, [tileTimers, grid]);
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4">
      ...
      {plankModalOpen && plankTileData && (
        <PlankPuzzleModal
          isOpen={plankModalOpen}
          onClose={() => {
            setPlankModalOpen(false);
            setPlankTileData(null);
          }}
            ...
            if (success && plankTileData) {
              ...
              try {
                ...
                await fetchAuthRetry('/api/property-timers', {
                  method: 'PUT',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ x: plankTileData.x, y: plankTileData.y, isReady: false, endTime: endIso, tileId: 'plank-labyrinth' })
                });
                window.dispatchEvent(new CustomEvent('kingdom-building-collected'));
              } catch (e) {}

              try {
                const { goldManager, expManager, statsService } = await loadManagers();
                goldManager.gainGold(gold, 'tile-collect:plank-labyrinth');
