...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom,0px))] sm:bottom-0 right-0 z-[100000] flex max-h-screen w-full flex-col-reverse p-4 pointer-events-none md:max-w-[420px]",
      className
    )}
    {...props}
  />
...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between overflow-hidden rounded-xl border p-0 shadow-xl transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Title
    ref={ref}
    className={cn(
      "text-sm font-semibold text-white leading-tight group-[.text-zinc-900]:text-zinc-900 group-[.text-zinc-950]:text-zinc-950 group-[.text-black]:text-zinc-950",
      className
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Description
    ref={ref}
    className={cn(
      "text-sm text-zinc-300 leading-relaxed mt-1 group-[.text-zinc-900]:text-zinc-900/90 group-[.text-zinc-950]:text-zinc-950/90 group-[.text-black]:text-zinc-950/90",
      className
    )}
    {...props}
  />
