...
// 1. Mermaid: Siren's Symphony
const MermaidGame = ({ gameState, setGameState, setReward }: MinigameProps) => {
    ...
    useEffect(() => {
        ...
        } else if (playbackIndex >= sequence.length && !isUserTurn && sequence.length > 0) {
            setIsUserTurn(true)
            setUserIndex(0)
        }
...
const IslandGame = ({ setGameState, onWeatherChange }: MinigameProps) => {
    const { toast } = useToast()
...
// 3. Crystal Cavern: Geode Breaker
const GeodeGame = ({ gameState, setGameState, setReward }: MinigameProps) => {
    const { toast } = useToast()
    ...
    }, [isActive, timeLeft, clicks, setGameState, setReward, toast])
    // Added extensive deps, but removed 'clicks' from logic to avoid restart loops if possible. 
    // Actually Logic REQUIRES checking clicks when time runs out.
    // The previous bug was "resets on unmount". With component moved out, this should be stable.
...
// 4. Jungle: Vine Swing
const VineGame = ({ gameState, setGameState, setReward }: MinigameProps) => {
    const { toast } = useToast()
    ...
    useEffect(() => {
        ...
        // Start from -PI/2 to start at 0? sin(-PI/2) = -1. (-1+1)=0. Yes.
        // Or just let it run. 
        // Logic: val = (sin() + 1) * 50. 
        // 0 -> 0. 50 -> 100. 100 -> 0.
        ...
        const interval = setInterval(() => {
            ...
            // Use offset to start at left (0)
            // sin starts at 0. (0+1)*50 = 50 (Middle).
            // We want start at 0?
            // sin(-PI/2) = -1. 
            // So phase shift: elapsed/200 - Math.PI/2
            const val = (Math.sin((elapsed / 200) - Math.PI / 2) + 1) * 50
            setPos(val)
        }, 16)
    ...
    return (
        <div className="flex flex-col items-center gap-6 py-4">
            ...
            <div className="w-full relative h-8 bg-gray-700 rounded-full overflow-hidden">
                ...
                {/* Marker */}
                <div
                    className="absolute top-0 bottom-0 w-2 bg-white z-10 shadow-[0_0_10px_white] transition-none"
                    style={{ left: `${pos}%` }}
                ></div>
            </div>
...
// 5. Ruins: Excavation
const RuinsGame = ({ gameState, setGameState, setReward }: MinigameProps) => {
    const { toast } = useToast()
    ...
    const dig = (idx: number) => {
        ...
        if (attempts <= 1) {
            // After this Update, attempts will be 0.
            // Wait slight delay to show result?
            setTimeout(() => {
                setGameState('success')
                // Calc total found (including just added)
                const total = found + (content === 'artifact' ? 50 : content === 'gold' ? 10 : 0)
                setReward(`${total} Gold Found`)
            }, 500)
        }
    }
...
// 6. Graveyard: Wager
const GraveyardGame = ({ gameState, setGameState, setReward }: MinigameProps) => {
    const { toast } = useToast()
...
// 7. Oasis: Mirage
const OasisGame = ({ setGameState, setReward }: MinigameProps) => {
    const { toast } = useToast()
    const drink = () => {
...
// 8. Farmland: Harvest
const FarmGame = ({ setGameState, setReward }: MinigameProps) => {
    const { toast } = useToast()
    ...
    useEffect(() => {
        ...
        // Timer to update UI
        const interval = setInterval(() => {
            // Force re-render to update progress bar if planted
            if (localStorage.getItem(PLANT_KEY)) {
                setPlantedAt(localStorage.getItem(PLANT_KEY))
                // Updating state with same value might not trigger render in strict effect?
                // But we want to re-calc progress.
