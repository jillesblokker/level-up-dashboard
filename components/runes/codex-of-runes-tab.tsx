...
export function CodexOfRunesTab() {
  ...
  const [activeView, setActiveView] = useState<'runes' | 'cipher' | 'word-hunt'>('runes');
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      {/* ─── 1. HERO BANNER & CONSOLIDATED STAT STRIP ───────────────────── */}
      <div className="relative rounded-3xl overflow-hidden border border-amber-500/40 bg-gradient-to-br from-zinc-950 via-[#130d06] to-zinc-950 p-5 sm:p-7 shadow-2xl">
        ...
        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-1.5 max-w-xl">
            ...
            <h2 className="text-2xl sm:text-3xl font-serif font-bold text-transparent bg-clip-text bg-gradient-to-r from-amber-200 via-amber-400 to-amber-100 flex items-center gap-3">
              <span>Rune codex</span>
              <span className="font-serif text-amber-400 text-sm sm:text-base hidden sm:inline">ᚠ ᚢ ᚦ ᚨ ᚱ ᚲ ᚷ ᚹ</span>
            </h2>
            <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed font-serif">
              24 historical Elder Futhark runes lie scattered across Thrivehaven. Seek them throughout the realm to awaken their ancient power.
            </p>
          </div>

          {/* Unified Compact Stat Strip (All 3 pillars at a glance) */}
          <div className="grid grid-cols-3 gap-2 w-full lg:w-auto shrink-0">
            ...
            >
              <div className="flex justify-between items-center text-[10px] sm:text-xs font-serif text-zinc-400 mb-1">
                <span>Runes</span>
                <span className="text-amber-300 font-bold">{unlockedCount}/{totalRunes}</span>
              </div>
              <Progress value={progressPercent} className="h-1.5 bg-zinc-950" />
            </button>

            <button
              type="button"
              onClick={() => setActiveView('cipher')}
              className={`p-2.5 sm:p-3 rounded-xl border text-left transition-all ${
                activeView === 'cipher'
                  ? 'bg-amber-500/15 border-amber-500/50 shadow-sm'
                  : 'bg-zinc-900/80 border-amber-500/20 hover:border-amber-500/40'
            ...
            >
              <div className="flex justify-between items-center text-[10px] sm:text-xs font-serif text-zinc-400 mb-1">
                <span>Cipher</span>
                <span className="text-amber-300 font-bold">{cipherProgress.percent}%</span>
              </div>
              <Progress value={cipherProgress.percent} className="h-1.5 bg-zinc-950" />
            </button>

            <button
              type="button"
              onClick={() => setActiveView('word-hunt')}
              className={`p-2.5 sm:p-3 rounded-xl border text-left transition-all ${
                activeView === 'word-hunt'
                  ? 'bg-emerald-500/15 border-emerald-500/50 shadow-sm'
                  : 'bg-zinc-900/80 border-emerald-500/20 hover:border-emerald-500/40'
              }`}
            >
              <div className="flex justify-between items-center text-[10px] sm:text-xs font-serif text-zinc-400 mb-1">
                <span>Words</span>
              ...
              <Progress value={wordHuntProgress.percent} className="h-1.5 bg-zinc-950" />
            </button>
          </div>
        </div>
        ...
        {isMaster && (
          <div className="mt-5 p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <Trophy className="w-5 h-5 text-amber-400 animate-bounce shrink-0" />
              <div>
                <p className="text-xs sm:text-sm font-bold text-amber-200 font-serif">Runic master of Thrivehaven</p>
                <p className="text-[11px] text-zinc-300">You have bound all 24 ancient Elder Futhark runes to your codex.</p>
              </div>
            </div>
            <Button
              variant="outline"
              ...
              className="border-amber-500/40 text-amber-300 hover:bg-amber-950 text-xs rounded-xl h-7 gap-1.5 w-full sm:w-auto font-medium"
            >
              <RotateCcw className="w-3 h-3" /> Re-hide for hunt
            </Button>
          </div>
        )}
      </div>

      {/* ─── 2. SUB-TABS NAVIGATION PILLS ─────────────────────────────── */}
      <div className="flex items-center gap-2 bg-zinc-950/90 border border-amber-900/30 p-1.5 rounded-2xl overflow-x-auto no-scrollbar">
        <button
          type="button"
          onClick={() => setActiveView('runes')}
          className={`flex items-center gap-2 py-2.5 px-4 rounded-xl text-xs font-serif font-bold transition-all whitespace-nowrap shrink-0 ${
            activeView === 'runes'
        ...
        >
          ...
          <span>Runes ({unlockedCount}/{totalRunes})</span>
        </button>
        <button
          type="button"
          onClick={() => setActiveView('cipher')}
        ...
        >
          ...
          <span>Ancient cipher ({cipherProgress.percent}%)</span>
        </button>
        <button
          type="button"
          onClick={() => setActiveView('word-hunt')}
        ...
        >
          ...
          <span>Word hunt ({wordHuntProgress.completedCount}/{wordHuntProgress.totalCount})</span>
        </button>
      </div>

      {/* ─── TAB 1: RUNES (24 TILES & ALPHABET KEY) ───────────────────── */}
      {activeView === 'runes' && (
        <div className="space-y-6">
          {/* Controls: Ætt selection pills + Alphabet key toggle button */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 custom-scrollbar w-full sm:w-auto">
              {[
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-serif font-bold transition-all whitespace-nowrap border ${
                      isActive
            ...
            <Button
              ...
              className="border-amber-500/30 text-amber-300 hover:bg-amber-950 text-xs rounded-xl h-8 gap-1.5 font-medium shrink-0"
            >
              <KeyRound className="w-3.5 h-3.5" />
              {showAlphabetKey ? 'Hide alphabet key' : 'Alphabet key'}
            </Button>
          </div>

          {/* Alphabet Key Drawer */}
          {showAlphabetKey && (
            <div className="relative rounded-3xl overflow-hidden border border-amber-500/30 bg-zinc-950 p-5 sm:p-6 shadow-xl space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <p className="text-xs text-zinc-400 font-serif">
                Sound and letter mappings. Golden glyphs represent runes bound to your codex.
              </p>
            ...
            {filteredRunes.map((rune) => {
              ...
              return (
                <div
                  key={rune.id}
                  onClick={() => handleRuneClick(rune, isUnlocked)}
                  className={`relative group rounded-2xl p-3.5 sm:p-5 border transition-all duration-300 cursor-pointer select-none flex flex-col justify-between min-h-[140px] sm:min-h-[160px] ${
                    isUnlocked
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-11 h-11 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center font-serif text-2xl sm:text-3xl font-bold transition-transform group-hover:scale-105 shrink-0 ${
                          isUnlocked
                    ...
                    {isUnlocked ? (
                      <Badge className="bg-emerald-500/10 text-emerald-400 border-emerald-500/30 text-[10px] uppercase font-medium shrink-0">
                        Bound
                      </Badge>
                    ) : (
                      <Badge className="bg-zinc-900 text-zinc-500 border-zinc-800 text-[10px] flex items-center gap-1 font-medium shrink-0">
                        <Lock className="w-2.5 h-2.5" /> Hidden
                      </Badge>
                    )}
                  </div>

                  <div className="mt-3 pt-2.5 border-t border-zinc-900 text-xs">
                    {isUnlocked ? (
                      <div className="space-y-1">
                        <p className="text-zinc-300 text-[11px] line-clamp-2 leading-relaxed font-serif">
                          {rune.description}
                        </p>
                        <div className="flex items-center gap-1 text-[10px] text-amber-500/80 pt-0.5 font-serif">
                      ...
                      <p className="text-zinc-400 text-[11px] italic line-clamp-2 leading-relaxed font-serif flex items-center gap-1">
                        <span>💡</span>
                        <span>&ldquo;{rune.hint}&rdquo;</span>
                      </p>
                    )}
                  </div>
                </div>
      ...
      {/* ─── TAB 2: ANCIENT CIPHER ────────────────────────────────────── */}
      {activeView === 'cipher' && (
        <div className="space-y-6">
          {/* Necrion Lore Card */}
          <div className="relative rounded-3xl overflow-hidden border border-emerald-500/30 bg-gradient-to-br from-zinc-950 via-[#0a120c] to-zinc-950 p-5 sm:p-7 shadow-2xl">
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-5 sm:gap-6">
              ...
              <div className="space-y-2.5 flex-1">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-serif font-bold text-base text-emerald-200">
                    Necrion
                  </span>
                  <Badge className="bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 text-[10px] px-2 py-0.5 flex items-center gap-1 font-medium">
                    <Feather className="w-3 h-3 text-emerald-400" />
                    Lorekeeper
                  </Badge>
                </div>

                <div className="relative p-3.5 sm:p-4 rounded-2xl bg-zinc-950/70 border border-emerald-500/20 shadow-inner">
                  <p className="text-xs sm:text-sm text-zinc-200 leading-relaxed font-serif italic">
                    &ldquo;Mortal traveler... beneath every cobblestone whispers the Elder Futhark. The ancients did not carve these glyphs for simple ornament—they etched them as mirrors to human perseverance. Only those with patient curiosity, unbroken daily discipline, and quiet bravery will sense where the runes sleep.
                  </p>
                  <p className="text-xs sm:text-sm text-zinc-300 leading-relaxed mt-2 font-serif">
                    Connect the glyphs to letters to decode our sacred creed, then inscribe words into the codex to awaken Crypto, the runic crocodile guardian.&rdquo;
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* Ancient Cipher Console */}
          <div className="relative rounded-3xl overflow-hidden border border-amber-500/40 bg-gradient-to-b from-zinc-950 via-[#140e06] to-zinc-950 p-5 sm:p-7 shadow-2xl space-y-5">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-amber-500/20">
              <div>
                ...
                <p className="text-xs sm:text-sm text-zinc-400 font-serif mt-1">
                  Connect elder runes to their alphabet letters to decode the secret creed of habit building.
                </p>
              </div>
            ...
            {/* Decipher Progress Bar */}
            <div className="p-3.5 rounded-2xl bg-zinc-900/80 border border-amber-500/20 space-y-2">
            ...
            <div className="p-4 sm:p-6 rounded-2xl bg-black/60 border border-amber-500/30 shadow-inner">
              <div className="flex flex-wrap gap-y-4 sm:gap-y-5 gap-x-5 sm:gap-x-8 items-center justify-center">
            ...
            {cipherSolved && (
              <div className="p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-amber-500/20 via-emerald-500/10 to-amber-500/20 border border-amber-500/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <Sparkles className="w-5 h-5 text-amber-300 animate-spin shrink-0" style={{ animationDuration: '6s' }} />
                  <div>
      ...
      {/* ─── TAB 3: WORD HUNT & RUNIC INSCRIBER ────────────────────────── */}
      {activeView === 'word-hunt' && (
        <div className="space-y-6">
          <div className="relative rounded-3xl overflow-hidden border border-emerald-500/40 bg-gradient-to-b from-zinc-950 via-[#0b140e] to-zinc-950 p-5 sm:p-7 shadow-2xl space-y-6">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-emerald-500/20">
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-xl sm:text-2xl font-serif font-bold text-emerald-100">
                    Word hunt
                  </h3>
                  {allWordTiersComplete && (
                ...
                <p className="text-xs sm:text-sm text-zinc-400 font-serif mt-1">
                  Inscribe words into the stone console to summon the legendary crocodile guardian Crypto.
                </p>
              </div>

              <div className="w-full sm:w-56 bg-zinc-900/80 border border-emerald-500/30 p-3 rounded-2xl flex flex-col gap-1.5 shadow-inner shrink-0">
                <div className="flex justify-between items-center text-xs font-medium text-zinc-300 font-serif">
                  <span className="text-zinc-400">Word shelves</span>
                  <span className="text-emerald-300 font-bold">{wordHuntProgress.completedCount} / {wordHuntProgress.totalCount}</span>
                </div>
            ...
            {/* Rune Inscriber Console */}
            <div className="p-4 sm:p-6 rounded-2xl bg-black/60 border border-emerald-500/30 space-y-4 shadow-inner">
              <div className="flex items-center justify-between text-xs font-medium font-serif">
                ...
                {WORD_HUNT_TIERS.map((tier) => {
                  ...
                  return (
                    <div
                      key={tier.length}
                      onClick={() => {
                        if (!isTierComplete && inputRef.current) {
                          inputRef.current.focus();
                      ...
                      className={`p-4 rounded-2xl border transition-all duration-300 flex flex-col justify-between gap-3 ${
                        isTierComplete
                          ...
                          : 'bg-zinc-950/60 border-zinc-800/80 hover:border-emerald-500/40 cursor-pointer group'
                      }`}
                    >
                      <div className="flex items-start justify-between gap-2">
                        ...
                        {isTierComplete ? (
                          ...
                          <Lock className="w-3.5 h-3.5 text-zinc-600 shrink-0 mt-0.5 group-hover:text-emerald-400 transition-colors" />
                        )}
                      </div>
            ...
            {/* Crypto Crocodile Guardian Unlock */}
            {allWordTiersComplete && (
              <div className="pt-6 border-t border-emerald-500/30 space-y-5 animate-in fade-in zoom-in-95 duration-500">
                <div className="text-center max-w-xl mx-auto space-y-1.5">
                  ...
                  <p className="text-xs sm:text-sm text-zinc-300 font-serif">
                    All eight tiers carved into the codex. Crypto has awakened and joined your citizens and creature collection!
                  </p>
                </div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] overflow-hidden rounded-3xl border-2 border-emerald-500/60 bg-zinc-950 flex flex-col shadow-2xl shadow-emerald-950/50 group-hover:border-emerald-400 group-hover:scale-[1.01] transition-transform">
                        ...
                        <div className="p-3 bg-zinc-950/90 border-t border-emerald-500/30 text-center">
                          <span className="text-xs font-serif font-bold text-emerald-300">
                            Crypto • Crocodile warrior
                          </span>
                        </div>
                      </div>

                      {/* BACK FACE */}
                      <div className="absolute inset-0 w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] overflow-hidden rounded-3xl border-2 border-emerald-500/60 bg-gradient-to-b from-zinc-950 via-[#07130a] to-zinc-950 p-6 flex flex-col justify-between shadow-2xl shadow-emerald-950/50">
                        <div className="space-y-4">
                          <div className="flex items-start justify-between border-b border-emerald-500/20 pb-3">
                            <div>
                              <h4 className="text-lg font-serif font-bold text-emerald-200">
                                Crypto
                              </h4>
                          ...
                          <div className="space-y-2 text-xs text-zinc-300 leading-relaxed font-serif">
                            <p className="italic text-emerald-200/90">
                              &ldquo;Snap! You cracked Necrion&apos;s codex! My armor is humming with ancient power!&rdquo;
                            </p>
                            <p className="text-[11px] text-zinc-400">
                              Slumbering beneath the stone archives, Crypto awakens when all word tiers are completed.
                            </p>
                          </div>
                        </div>
