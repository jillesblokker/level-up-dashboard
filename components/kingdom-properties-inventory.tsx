...
export function KingdomPropertiesInventory({
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-zinc-950 " role="dialog" aria-modal="true">
            ...
            <TabsList className="w-full grid grid-cols-5 bg-[#1a1d24] p-1 h-10 rounded-lg border border-amber-900/20 gap-0.5">
              {[
                ...
                <TabsTrigger
                  key={tab.value}
                  value={tab.value}
                  className="data-[state=active]:bg-[#2a2e37] data-[state=active]:text-amber-400 text-zinc-500 font-medium text-[11px] transition-all px-1"
                >
                  {tab.label}
                </TabsTrigger>
              ))}
          ...
          <ScrollArea className="flex-1 px-6 pb-6">
            ...
            <TabsContent value="forge" className="mt-4">
              ...
              <div className="flex items-center justify-between bg-[#0a0c10] border border-amber-900/20 rounded-xl px-4 py-3 mb-4">
                <span className="text-sm text-zinc-400 font-medium">Your Gold</span>
  ...
  return (
    <Card
      className={cn(
        ...
        isPlaceMode
          ...
          : isLocked
            ? "border-zinc-800 opacity-70 grayscale-[0.5]"
            : isAffordable
    ...
    >
      ...
      <CardContent className="p-3 flex-1 flex flex-col">
        <h3 className={cn("font-bold text-lg leading-tight mb-0.5 truncate font-medieval tracking-wide text-center", isLocked ? "text-zinc-500" : "text-amber-100")}>
          {tile.name}
        </h3>
        {!isLocked && (
        ...
        <div className="mt-auto space-y-3">
          <div className="flex flex-col gap-1.5 bg-zinc-950 rounded-lg p-2.5 border border-white/5 shadow-inner">
                  ...
                  return (
                    <div key={idx} className="flex flex-col gap-0.5">
                      <div className="flex justify-between text-[10px]">
                        <span className={isEnough ? "text-green-500" : "text-zinc-400"}>{mat.quantity}x {matName}</span>
