...
export async function saveToSupabaseClient<T>(
  ...
  try {
    ...
    if (response.ok) {
      // logger.debug(`[Supabase Persistence Client] ✅ Data saved to Supabase successfully: ${endpoint}`);
      setUserScopedItem(localStorageKey, JSON.stringify(data));
      return true;
    } else {
      const errorText = await response.text();
      // logger.warn(`[Supabase Persistence Client] ⚠️ Failed to save to Supabase, falling back to localStorage: ${endpoint}`, {
    ...
    // logger.error(`[Supabase Persistence Client] Error saving to Supabase: ${endpoint}`, error);
    setUserScopedItem(localStorageKey, JSON.stringify(data));
    return false;
  }
}
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (result.stats || result.progress || result.grid || result.timers || result.items || result.states || result.tiles) {
        // logger.debug(`[Supabase Persistence Client] ✅ Data loaded from Supabase: ${endpoint}`);
    ...
    logger.debug(`[Supabase Persistence Client] ⚠️ Failed to load from Supabase, using localStorage: ${endpoint}`);
    const stored = getUserScopedItem(localStorageKey);
    return stored ? JSON.parse(stored) : defaultValue;
  } catch (error) {
    // logger.error(`[Supabase Persistence Client] Error loading from Supabase: ${endpoint}`, error);
    const stored = getUserScopedItem(localStorageKey);
    return stored ? JSON.parse(stored) : defaultValue;
  }
}
