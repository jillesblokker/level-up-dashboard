import { saveToSupabase, loadFromSupabase } from './supabase-persistence';
...
export async function loadCharacterStats(): Promise<CharacterStats> {
  ...
  return await loadFromSupabase('/api/character-stats', 'character-stats', defaultStats);
}

/**
 * Saves character stats to both Supabase and localStorage
 */
export async function saveCharacterStats(stats: CharacterStats): Promise<boolean> {
  return await saveToSupabase('/api/character-stats', { stats }, 'character-stats');
}
